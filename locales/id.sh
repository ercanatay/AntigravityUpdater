# Indonesian (Bahasa Indonesia)
LANG_NAME="Bahasa Indonesia"
LANG_CODE="id"

MSG_TITLE="🚀 Pembaruan Antigravity Tools"
MSG_CHECKING_VERSION="📦 Memeriksa versi saat ini..."
MSG_CURRENT="Saat ini"
MSG_NOT_INSTALLED="Belum terpasang"
MSG_UNKNOWN="Tidak diketahui"
MSG_CHECKING_LATEST="🌐 Memeriksa versi terbaru..."
MSG_LATEST="Terbaru"
MSG_ARCH="Arsitektur"
MSG_ALREADY_LATEST="✅ Anda sudah memiliki versi terbaru!"
MSG_NEW_VERSION="📥 Versi baru tersedia! Memulai unduhan..."
MSG_DOWNLOADING="⬇️  Mengunduh DMG..."
MSG_DOWNLOAD_FAILED="❌ Unduhan gagal!"
MSG_DOWNLOAD_COMPLETE="✅ Unduhan selesai"
MSG_MOUNTING="💿 Memasang DMG..."
MSG_MOUNT_FAILED="❌ Gagal memasang DMG"
MSG_MOUNTED="✅ DMG terpasang"
MSG_CLOSING_APP="🔄 Menutup aplikasi saat ini..."
MSG_REMOVING_OLD="🗑️  Menghapus versi lama..."
MSG_COPYING_NEW="📁 Menyalin versi baru..."
MSG_APP_NOT_FOUND="❌ Aplikasi tidak ditemukan di DMG"
MSG_COPIED="✅ Aplikasi tersalin"
MSG_REMOVING_QUARANTINE="🔓 Menghapus karantina (xattr -cr)..."
MSG_QUARANTINE_REMOVED="✅ Karantina dihapus"
MSG_UNMOUNTING="💿 Melepas DMG..."
MSG_UPDATE_SUCCESS="✅ PEMBARUAN BERHASIL!"
MSG_OLD_VERSION="Versi lama"
MSG_NEW_VERSION_LABEL="Versi baru"
MSG_API_ERROR="❌ Tidak dapat mengakses GitHub API"
MSG_SELECT_LANGUAGE="Pilih bahasa"
MSG_OPENING_APP="🚀 Membuka aplikasi..."
# Backup & Rollback
MSG_BACKUP_CREATED="✅ Backup created"
MSG_BACKUP_FAILED="⚠️  Backup failed"
MSG_ROLLBACK_SUCCESS="✅ Rollback successful"
MSG_ROLLBACK_FAILED="❌ Rollback failed"
MSG_NO_BACKUP="❌ No backup found"

# Integrity & Code Signing
MSG_HASH_VERIFY="🔍 Verifying file integrity..."
MSG_HASH_OK="✅ File integrity verified"
MSG_HASH_FAILED="❌ File integrity check failed!"
MSG_CODESIGN_CHECK="🔐 Checking code signature..."
MSG_CODESIGN_OK="✅ Code signature valid"
MSG_CODESIGN_WARN="⚠️  Warning: Code signature not verified"

# Auto Update
MSG_AUTO_UPDATE_ENABLED="✅ Pembaruan otomatis diaktifkan"
MSG_AUTO_UPDATE_DISABLED="✅ Pembaruan otomatis dinonaktifkan"
MSG_AUTO_UPDATE_INVALID_FREQ="❌ Frekuensi pembaruan otomatis tidak valid"
MSG_AUTO_UPDATE_SUPPORTED="Nilai yang didukung: hourly, every3hours, every6hours, daily, weekly, monthly"
# New Features (v1.7.0)
MSG_VERSION_PINNED="📌 Installing specific version"
MSG_SELF_UPDATE_CHECKING="🔄 Checking for updater updates..."
MSG_SELF_UPDATE_AVAILABLE="📦 Updater update available"
MSG_SELF_UPDATE_SUCCESS="✅ Updater updated successfully"
MSG_SELF_UPDATE_CURRENT="✅ Updater is up to date"
MSG_HOOK_PRE_UPDATE="⚙️  Running pre-update hook..."
MSG_HOOK_POST_UPDATE="⚙️  Running post-update hook..."
MSG_HOOK_FAILED="❌ Hook script failed"
MSG_NOTIFICATION_SENT="🔔 Notification sent"
MSG_DOWNLOAD_RETRY="🔄 Retrying download..."
MSG_HISTORY_TITLE="📋 Update History"
MSG_HISTORY_EMPTY="No update history"
MSG_CACHE_HIT="✅ Cached version matches, skipping download"
